...
description: Everything needed to build, test and review the hass-iopool integration — architecture, entity patterns, invariants, Home Assistant expectations, and the deliberate deviations this project has taken. Primary reference for any work in custom_components/iopool/.
---

# Home Assistant Integration Knowledge — hass-iopool

Use this skill for any implementation, review, or debugging task in the `custom_components/iopool/` codebase. Sections 1 to 9 cover this project; §10 covers what Home Assistant expects of any integration, and the two places where this project deliberately departs from it.

---

## 1. Project Stack

- **Language**: Python 3.14 (required by Home Assistant since 2026.3), `async`/`await` everywhere for I/O
...
- **Linter**: `ruff` — `ruff check` and `ruff format --check`, configured in `custom_components/iopool/pyproject.toml`. Line length 88, `E501` deliberately ignored. CI pins `ruff==0.16.1`.
...
- Test rules: what to test per entity, ruff gate
...
If any of the following is unclear, gather the information with `AskUserQuestion` before starting implementation.
...
4. **When in doubt, ask the developer** — if it is unclear whether a change warrants a documentation update, ask with `AskUserQuestion` before writing or skipping it:
...
- Run `ruff check custom_components/iopool` and `ruff format --check custom_components/iopool`
- Do not flag line length: `E501` is disabled on purpose, the formatter owns it
- Do not flag `except ValueError, TypeError:` — PEP 758 is valid on Python 3.14 and is what `ruff format` produces here
...
### Step 3 — Analyze via the GitHub CLI

`gh` is installed. Use it against `home-assistant/core` (Python backend, entities, platforms, helpers) or `home-assistant/frontend` (UI):

```bash
gh api repos/home-assistant/core/commits --paginate -f path=homeassistant/components/sensor/__init__.py
gh api repos/home-assistant/core/commits/<sha>
gh search code 'MyClass repo:home-assistant/core'
gh search issues 'breaking change sensor coordinator' --repo home-assistant/core
gh pr list --repo home-assistant/core --search 'sensor coordinator' --state merged
```

Reading a merged upstream PR body is often faster than reading its diff: it usually states the intent and whether a breaking change was declared.
...
## 10. Home Assistant Expectations

These apply to any Home Assistant integration, not just this one. They come from upstream review practice and are worth honouring even though this integration ships through HACS rather than core.

### 10.1 Configuration

- **Polling intervals are not user-configurable.** Never add `scan_interval`, `update_interval` or a polling-frequency option to the config flow or the options flow. `DEFAULT_SCAN_INTERVAL` in `const.py` is the single source.
- **Do not let users name the config entry** in the config flow. The name is derived, and can be changed later in the UI.

### 10.2 Entities

- `async_added_to_hass()` and `async_will_remove_from_hass()` must be symmetrical. A subscription created in one is torn down in the other. `IopoolElapsedFiltrationSensor` uses `self.async_on_remove(...)` for exactly this.
- Do not redeclare or duplicate attributes, properties or methods the entity base class already provides, and do not guard against the parent's behaviour changing. Rely on the base class.

### 10.3 Validation and defensive code

- Do not add defensive checks for values Home Assistant has already validated through a service schema or an entity selector. Add guards only where data bypasses those validators, or is transformed into a less safe form.
- Where validation guarantees a key exists, index it directly (`data["key"]`) rather than `.get("key")`, so a contract violation surfaces instead of being masked.

### 10.4 Libraries

Do not implement workarounds in the integration for bugs or limitations in a dependency — fix the dependency instead. This integration has no runtime dependency, so in practice the rule applies to Home Assistant itself: when HA changes behaviour, adapt to the new contract rather than patching around it, and investigate the upstream change first (§9).

### 10.5 Diagnostics and repairs

- `diagnostics.py` must never expose the API key, tokens or precise coordinates. `TO_REDACT` covers `api_key`; extend it whenever a new secret enters the payload.
- There is no `repairs.py` today. If one is added, every issue must be actionable: state what is happening, why it matters, the exact steps to resolve it, and what to expect afterwards. `CRITICAL` is for extreme cases only, `ERROR` needs immediate attention, `WARNING` signals future breakage. Never raise an issue the user cannot act on.

### 10.6 Finding examples

When looking for a pattern in Home Assistant core, prefer integrations with a `platinum` or `gold` quality scale in their `manifest.json` — they are held to the highest standard.

### 10.7 Tests

Avoid asserting on internal integration details. Test observable behaviour: entity states, published events, service calls, config entry state.

---

## 11. Deliberate Deviations

Two upstream expectations do **not** apply here. Do not raise them in review.

### 11.1 No wrapper library

Home Assistant expects integrations to be thin wrappers, with protocol handling and domain logic in a separate PyPI package. This integration calls the iopool API directly with `aiohttp` from `coordinator.py`, and `manifest.json` declares `"requirements": []`.

This is a considered choice, not an oversight: the extractable surface — a handful of HTTP calls and four dataclasses — was too small to justify a dedicated package and its release cycle. Do not flag the inline API calls as an architecture defect, and do not propose extracting a library unless the API surface grows substantially.

### 11.2 `quality_scale: bronze` is a target, not a claim

`manifest.json` declares `quality_scale: bronze`, and there is no `quality_scale.yaml`. The declaration dates from a plan to publish into Home Assistant core; the project went to HACS instead, for release flexibility.

Treat the bronze rules as a code-cleanliness goal worth converging on, not as a statement of current compliance. hassfest does not enforce `quality_scale.yaml` for custom integrations, so nothing in CI will tell you where the gaps are — see the `ha-quality-scale-verify` skill to audit a rule.
