...
def hass():
    ...
    # Add loop for event tracking (required by Home Assistant)
    try:
        hass_mock.loop = asyncio.get_running_loop()
    except RuntimeError:
        # No event loop running, create one for the mock
        hass_mock.loop = asyncio.new_event_loop()
...
def suppress_aiohttp_warnings():
    """Suppress aiohttp unclosed session warnings in tests."""
    # Filter out aiohttp unclosed session warnings
    ...
    # Also suppress asyncio warnings
    warnings.filterwarnings(
        "ignore",
        message=".*was never awaited",
        category=RuntimeWarning,
    )
    # Suppress Home Assistant deprecation warnings during tests
    warnings.filterwarnings(
        "ignore",
    ...
    warnings.filterwarnings(
        "ignore",
        message="Setting custom ClientSession.close attribute is discouraged",
        category=DeprecationWarning,
    ...
    # Reset warnings filters after test
